---
name: crm_data_enrichment_agent
description: "Enrich CRM contacts and B2B leads with company data"
version: 1.0.0
...
Skill package: `crm_data_enrichment_agent`
...
- Clean and enrich CRM contact lists with public web signals and third‑party enrichment.
- Extract company and website metadata (titles, descriptions, contact pages, addresses).
- Reduce manual research by automating website crawling for contact and company signals.

## Inputs Expected From Normal Users

- `emailField` (string) - Property or column name for the contact's email in your data.
...
- Actor slug: `solutionssmart/crm-data-enrichment-agent`
- Required secret: `APIFY_TOKEN`
- Start run endpoint: `https://api.apify.com/v2/acts/solutionssmart~crm-data-enrichment-agent/runs`
- Run-sync endpoint: `https://api.apify.com/v2/acts/solutionssmart~crm-data-enrichment-agent/run-sync-get-dataset-items`
...
- High-cost fields: `maxContacts`, `enrichment`
...
- "Use `crm_data_enrichment_agent` to run the default workflow for this source."
- "Generate a request for `crm_data_enrichment_agent` using these parameters: ..."
- "Validate this `crm_data_enrichment_agent` input before running it."
- "Retry the failed `crm_data_enrichment_agent` operation and explain what changed."

## Integration Notes For Hermes Agent

Copy this folder into `~/.hermes/skills/<category>/crm_data_enrichment_agent/` or install it with `hermes skills install <url>`. Keep `SKILL.md`, `manifest.json`, and `input_schema.json` together so the agent can load the instructions and validate user input.
