---
name: co_star_real_estate_scraper
description: "Scrapes publicly accessible pages from CoStar News GB and"
version: 1.0.0
...
Skill package: `co_star_real_estate_scraper`
...
- Export extracted property, transaction, media and company data to BI tools, CRMs, spreadsheets, dashboards or enrichment pipelines.
- Automate recurring monitoring of new CoStar property news for market monitoring, lead generation, investment research and deal tracking.

## Inputs Expected From Normal Users

- `startUrls` (array) - Public CoStar listing or section pages to seed the crawl.
- `maxItems` (integer) - Maximum number of records to store in the dataset.
...
- Actor slug: `solutionssmart/co-star-real-estate-scraper`
- Required secret: `APIFY_TOKEN`
- Start run endpoint: `https://api.apify.com/v2/acts/solutionssmart~co-star-real-estate-scraper/runs`
- Run-sync endpoint: `https://api.apify.com/v2/acts/solutionssmart~co-star-real-estate-scraper/run-sync-get-dataset-items`
...
`maxConcurrency`, `desiredConcurrency`, `proxyConfiguration`, `proxyInitializationTimeoutSecs`, `fallbackToNoProxyOnProxyError`
...
`maxItems`, `maxRequestsPerCrawl`, `maxConcurrency`, `desiredConcurrency`, `maxPaginationPages`, `proxyConfiguration`, `proxyInitializationTimeoutSecs`, `fallbackToNoProxyOnProxyError`
...
- High-cost fields: `maxItems`, `maxRequestsPerCrawl`, `maxConcurrency`, `desiredConcurrency`, `maxPaginationPages`, `proxyConfiguration`, `proxyInitializationTimeoutSecs`, `fallbackToNoProxyOnProxyError`
...
- "Use `co_star_real_estate_scraper` to run the default workflow for this source."
- "Generate a request for `co_star_real_estate_scraper` using these parameters: ..."
- "Validate this `co_star_real_estate_scraper` input before running it."
- "Retry the failed `co_star_real_estate_scraper` operation and explain what changed."

## Integration Notes For Hermes Agent

Copy this folder into `~/.hermes/skills/<category>/co_star_real_estate_scraper/` or install it with `hermes skills install <url>`. Keep `SKILL.md`, `manifest.json`, and `input_schema.json` together so the agent can load the instructions and validate user input.
